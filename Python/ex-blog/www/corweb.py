...
def has_request_arg(fn):
    ...
    for k, v in params.items():
        if k == 'request':
            found = True
            continue
        if found and (v.kind != inspect.Parameter.VAR_POSITIONAL and v.kind != inspect.Parameter.KEYWORD_ONLY and v.kind != inspect.Parameter.VAR_KEYWORD):
...
# POSITIONAL_OR_KEYWORD  通过位置或关键字 func(a, b, c)
# VAR_POSTIONAL          通过位置         func(*a)
# KEYWORK_ONLY           通过关键字       func(*a, b, c)
# VAR_KEYWORD            可变关键字       func(**a)
            ...
            if request.method == 'GET':
                ...
                if qs:
                    ...
                    for k, v in parse.parse_qs(qs, True).items():
                        kw[k] = v[0]
        # filter parameters
        logging.info('RequestHandler:%s' % str(kw))
        if kw is None:
